...
import net.mcreator.skarriermobs.procedures.FloreHeadOnEntityTickUpdateProcedure;
...
@Mixin(value = FloreHeadOnEntityTickUpdateProcedure.class, remap = false)
public abstract class MixinSkarrierFloreHeadScans {
    ...
    private static List<Entity> coo$narrowFloreHeadScans(LevelAccessor level, Class<Entity> type, AABB box,
        ...
        if (type == Entity.class) {
            ...
            if (size == 2.0D && CoOConfig.skarriermobsLeanTargetProximityScans) {
                return SkarrierScans.targetIn(self, box, filter);
            }
            if (size == 32.0D && CoOConfig.skarriermobsNarrowRegionScans) {
