...
import net.mcreator.skarriermobs.procedures.CarnagerOnEntityTickUpdateProcedure;
...
@Mixin(value = CarnagerOnEntityTickUpdateProcedure.class, remap = false)
public abstract class MixinSkarrierCarnagerScans {
    ...
    private static List<Entity> coo$narrowCarnagerScans(LevelAccessor level, Class<Entity> type, AABB box,
        ...
        if (type == Entity.class) {
            ...
            if (size == 6.0D && CoOConfig.skarriermobsLeanTargetProximityScans) {
                return SkarrierScans.targetIn(self, box, filter);
            }
            if (size == 20.0D && CoOConfig.skarriermobsNarrowRegionScans) {
